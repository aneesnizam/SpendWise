...
export default function Forgetpass({ onForgetPassword }) {
  ...
  const { setForgetPassword } = userlogindata();
  ...
  const handleSendCode = async () => {
    ...
    try {
      await api.post("api/auth/forgot-password", { email }).then((res) => {
  ...
  const handlereset = async () => {
    ...
    try {
      await api
        .post("api/auth/reset-password", { email, otp, newPassword: password })
        .then((res) => {
          if (res.data.success) {
            toast.success(res.data.message);
          } else {
